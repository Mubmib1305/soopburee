...
def page5_2_3():
    result = supabase.from_('guardians').select('selected_flower').eq('id', session['user_id']).execute()
    selected_flower = result.data[0]['selected_flower'] if result.data else 1
    return render_template('case2/page5_2_3.html', selected_flower=selected_flower)
...
# ไฟล์นี้จัดการเส้นทาง (routes) สำหรับส่วนแสดงผลบนอุปกรณ์มือถือ
# ทำหน้าที่:
# 1. กำหนดเส้นทางพื้นฐานสำหรับการนำทางในแอป:
#    - หน้าหลัก, หน้าลงทะเบียน, หน้าแสดงชื่อผู้ใช้, หน้าเลือกวิธีการดูแล ฯลฯ
# 2. กำหนดเส้นทางสำหรับกรณีศึกษาทั้งสองแบบ:
#    - Case 1: เส้นทาง /case1/page5_1_1 ถึง /case1/page5_1_11
#    - Case 2: เส้นทาง /case2/page5_2_1 ถึง /case2/page5_2_8
# 3. จัดการ API endpoints สำหรับการทำงานกับฐานข้อมูล Supabase:
#    - /api/register: ลงทะเบียนผู้ใช้งาน
#    - /api/select-flower: เลือกดอกไม้
#    - /api/select-care-method: เลือกวิธีการดูแล
#    - /api/save-message: บันทึกข้อความ
#    - /api/select-quote: เลือกคำคม
#    - และอื่นๆ
# 4. จัดการการเชื่อมต่อกับ Supabase และจัดการข้อผิดพลาดทั้งหมด
# 5. ใช้ session ในการเก็บข้อมูลผู้ใช้ระหว่างการใช้งาน